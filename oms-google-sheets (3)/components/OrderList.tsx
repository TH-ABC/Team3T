...
import { Search, Plus, RefreshCw, Copy, ArrowUp, ArrowDown, Save, ExternalLink, Calendar, FileSpreadsheet, ChevronLeft, ChevronRight } from 'lucide-react';
...
const OrderList: React.FC<OrderListProps> = ({ user }) => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [orderResult, storeData] = await Promise.all([
          sheetService.getOrders(selectedMonth), // Gọi hàm mới trả về object { orders, fileId }
          sheetService.getStores()
      ]);
      setOrders(orderResult.orders);
      setCurrentFileId(orderResult.fileId); // Lưu File ID
      setStores(storeData);
    } catch (e) {
      console.error(e);
      setOrders([]);
      ...
      <div className="bg-white shadow-sm overflow-hidden rounded-lg">
        ...
        <div className="p-4 border-b border-gray-200 flex flex-col xl:flex-row justify-between items-center gap-4">
          <div className="flex flex-col md:flex-row items-center gap-4 w-full xl:w-auto">
            ...
            {/* Bộ chọn tháng và Link File - DESIGN MỚI VỚI DROPDOWN NĂM */}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? 
                <tr><td colSpan={10} className="text-center py-12 text-gray-500">Đang tải dữ liệu tháng {selectedMonth}...</td></tr> : 
                (sortedOrders.length === 0 ? 
                    <tr><td colSpan={10} className="text-center py-12 text-gray-500">Tháng {selectedMonth} chưa có đơn hàng nào.<br/><span className="text-xs text-gray-400">Nếu bạn chắc chắn đã có đơn, hãy kiểm tra lại bộ chọn tháng hoặc quyền truy cập.</span></td></tr> :
                    sortedOrders.map((order, idx) => (
                        <tr key={order.id || idx} className="hover:bg-gray-50 border-b border-gray-200 text-gray-800 transition-colors">
                            {/* DATE & ID & STORE: Read Only */}
            ...
            <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl transform transition-all scale-100">
                ...
                <form onSubmit={handleCreateOrder} className="p-6">
                    <div className="space-y-4">
                        <div>
                            <label className="block text-sm font-bold text-gray-800 mb-1">Ngày Đặt (Date) <span className="text-red-500">*</span></label>
                            <input 
                                type="date" 
                                required 
                                className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-[#1a4019] focus:border-transparent placeholder-gray-400"
                                value={newOrder.date} 
                                onChange={(e) => setNewOrder({...newOrder, date: e.target.value})} 
                            />
                             <p className="text-xs text-gray-500 mt-1">Đơn hàng sẽ được lưu vào file của tháng tương ứng.</p>
                        </div>

                        <div>
                            <label className="block text-sm font-bold text-gray-800 mb-1">Mã Đơn Hàng (ID) <span className="text-red-500">*</span></label>
                            <input 
                                type="text" 
                                required 
                                className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-[#1a4019] focus:border-transparent placeholder-gray-400"
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <select 
                                    required 
                                    className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-[#1a4019] focus:border-transparent appearance-none cursor-pointer"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-bold text-gray-800 mb-1">Loại (Type)</label>
                                <input 
                                    type="text" 
                                    className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-[#1a4019] placeholder-gray-400" 
                                    placeholder="T-Shirt..." 
                                    value={newOrder.type} 
                            ...
                            <div>
                                ...
                                <input 
                                    type="number" 
                                    className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-[#1a4019] text-center font-bold" 
                        ...
                        <div>
                            ...
                            <input 
                                type="text" 
                                className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-[#1a4019] placeholder-gray-400 font-mono text-sm" 
                        ...
                        <div>
                            ...
                            <textarea 
                                className="w-full border border-gray-300 rounded-md px-4 py-2 text-gray-900 bg-white focus:outline-none focus:ring-2 focus:ring-[#1a4019] placeholder-gray-400 text-sm resize-none" 
                                rows={3}
                                placeholder="Nhập ghi chú (màu, size đặc biệt)..." 
                                value={newOrder.note} 
